...
- 新增 [catalog-source.test.ts](/Users/pray/project/ai_pm_library/app/api/catalog-source.test.ts)，专责 `generate-catalog` 的正向/负向校验。
...
- 新增 `/Users/pray/project/ai_pm_library/app/scripts/catalog-source-contract.schema.json`，定义 contract 的元结构与关键字段约束（集合、校验字段、分类列表、生成来源等）。
