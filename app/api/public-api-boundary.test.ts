...
describe("phase 0 public boundary", () => {
  ...
  it("uses container-side MySQL password expansion in compose healthcheck", () => {
    const compose = readRepoFile("deploy/docker-compose.yml");

    expect(compose).toContain("--password=$$MYSQL_PASSWORD");
    expect(compose).not.toContain("--password=${MYSQL_PASSWORD}");
  });
});
